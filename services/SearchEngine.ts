        ...
        try {
            ...
            const getUniqueKey = (book: GoogleBookResult) => {
                if (book.volumeInfo.industryIdentifiers?.length) {
                    const isbn = book.volumeInfo.industryIdentifiers.find(id => id.type.includes("ISBN"))?.identifier;
...
function calculateRelevanceScore(
    ...
    const normalizedAuthors = normalizeForMatching(
        info.authors?.join(" ") || "",
    );
    ...
    if (info.industryIdentifiers && info.industryIdentifiers.length > 0) score += 5;
