...
export const BackupService = {
    ...
    saveToDevice: async (books: Book[]) => {
        if (Platform.OS !== 'android') {
            Alert.alert('Bilgi', 'Bu özellik sadece Android cihazlarda kullanılabilir. Lütfen "Paylaş" seçeneğini kullanın.');
            return;
        }
        ...
        try {
            ...
            Alert.alert('Başarılı', `Yedek dosyası başarıyla kaydedildi:\n${fileName}`);
    ...
    restoreBackup: async (): Promise<Book[] | null> => {
        try {
            ...
            if (!parsedData.books || !Array.isArray(parsedData.books)) {
                Alert.alert('Geçersiz Dosya', 'Seçilen dosya geçerli bir yedek dosyası değil.');
                return null;
            }
