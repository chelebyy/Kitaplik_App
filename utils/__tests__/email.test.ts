...
describe('createFeedbackMailto', () => {
  ...
  it('should include body if provided', () => {
    const url = createFeedbackMailto('test@example.com', 'Test Subject', 'Hello World');
    expect(url).toBe('mailto:test@example.com?subject=Test%20Subject&body=Hello%20World');
  });

  it('should handle special characters in subject and body', () => {
    const url = createFeedbackMailto('test@example.com', 'Hata & İstek', 'Satır 1\nSatır 2');
